    ...
    <section className="editor-panel">
      ...
      <form onSubmit={event => { event.preventDefault(); void save("draft"); }}>
        <fieldset disabled={busy} className="space-y-6">
          ...
          <label className="editor-label" htmlFor="story-date">Date of the update<Input id="story-date" type="date" value={editor.story_date ?? ""} onChange={event => update("story_date", event.target.value)} /><span className="editor-help">Optional event date. Otherwise the publication date is shown.</span></label><details><summary className="cursor-pointer text-sm font-medium">Story address</summary><div className="space-y-4 mt-4"><label className="editor-label" htmlFor="story-slug">Story address<Input id="story-slug" value={editor.slug} maxLength={120} onChange={event => { setCustomSlug(true); update("slug", event.target.value); }} /><span className="editor-help">/blog/{editor.slug || "your-story"} — changing a published address breaks old links.</span></label></div></details>
        </fieldset>
      </form>}
